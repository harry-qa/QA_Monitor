...
- **크래시 덤프 자동 분석**: WER(Windows Error Reporting)이 캡처한 덤프를 ClrMD 기반 헬퍼(`DumpAnalyzer`)로 분석해 관리 코드 스택 트레이스를 크래시 이력에 자동 첨부 (AccessViolation류처럼 이벤트 로그만으로는 스택이 안 남는 크래시용). 덤프 파일명의 PID로 정확한 크래시 이벤트에 연결해, 같은 앱이 여러 인스턴스로 돌거나 짧은 시간에 연속 크래시해도 다른 크래시의 스택이 잘못 붙지 않음
...
- **셸 확장(호스트 대행) 행 식별**: 이지랩 셸 확장(예: ezZip 탐색기 통합 `ShellExtensionWin11.dll`)은 자기 exe가 아니라 COM Surrogate(`dllhost.exe`)/`rundll32.exe` 안에서 실행돼, 멈추면 이벤트/이력에 껍데기 프로세스명(dllhost.exe)으로 찍힌다. 패키지 앱 ID·로드된 모듈 경로로 실제 제품을 식별해 `ezZip (셸 확장)`처럼 이름을 바로잡고, 그 호스트가 이지랩 모듈을 물고 멈추면 실시간으로 전체 메모리 덤프를 캡처한다(무관한 dllhost는 제외). 네이티브 셸 확장 덤프는 관리 코드 스택이 없으므로 보고서 ZIP의 `.dmp`를 디버거로 분석
...
- **앱 버전 기록**: APPCRASH(1000)·AppHang(1002) 이벤트에서 크래시 당시 앱 버전을 함께 추출·저장하고 보고서에 표시 (과거 크래시엔 소급 불가하므로 지금부터 수집 — 이후 버전별 품질 분석의 기반)
- **자가 헬스(감시 신뢰성) 기록**: 모니터가 "놓쳤을 수 있는" 상태 — 이벤트 로그 초기화로 인한 수집 공백, 덤프 분석 반복 실패 후 포기 — 를 조용히 넘기지 않고 별도 기록해 상태 점검 창의 "감시 신뢰성 기록" 섹션에 표시. QA 도구에서 가장 위험한 "조용한 멈춤"을 드러냄
...
- 크래시/설치 이력·자가 헬스 기록은 `%LOCALAPPDATA%\ezLab QA Monitor\`에 원자적 쓰기로 영구 저장(강제종료 시에도 손상 방지), 이력은 최근 500건·헬스 기록은 최근 200건만 보관
- 크래시 덤프는 `%ProgramData%\ezLab QA Monitor\Dumps\`에 최근 20개만 보관, 오래된 것은 자동 삭제(중단된 쓰기가 남긴 임시파일도 함께 정리)
